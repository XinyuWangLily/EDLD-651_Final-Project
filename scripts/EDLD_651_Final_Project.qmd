...
First off, great work Xinyu, Sophie, and Everett! Interesting topic, too. Alrighty, let’s get to it!

[**Areas of Strength**]{.underline}

1.  Great project structure with easy-to-follow sections. Makes it easy to reproduce as well.

<!-- -->

2.  Good combo of using Tidyverse with here/readxl/janitor for loading and cleaning purposes.

<!-- -->

3.  Clean and clear visualizations with labeled axes to better understand relationships.

<!-- -->

4.  Nice reporting with linear models as they are fit and with summaries and tables to better present the results.

*Kindly find the rest of my comments at the very end.*
...
[**Things to Learn From**]{.underline}

1.  As mentioned earlier, really good combination of code-folding, which I honestly had not thought about for my own code, as well as inline outputs and a clean table of contents. This makes sharing results that much easier, especially when you code and write-up.
2.  Running summary() and check_model(), instead of just lm(), makes readable tables and outputs, which is good analytical practice to learn. A sort of fit, diagnose, visualize, and report pattern.

[**Areas for Potential Improvement**]{.underline}

*Writing this section, acknowledging the fact that I do not claim to know more about R than any of you, and these are simply potential ways for improvement :)*

1.  The regression outputs show “(80 observations deleted due to missingness)”. The script could first report how many rows/variables are missing, then explain exclusion versus imputation decisions (e.g., drop_na() vs impute), and finally, there could be sensitivity checks. Perhaps add a short table or skimr summary so readers know the scope of missing data.
2.  There appear to be repeated steps (data joins/tidy sequences/plotting code). You want to convert recurring patterns into small helper functions (e.g., make_scatter(df, x, y, xlab, ylab)) and add concise comments describing each major choice as to why transform or filter or certain covariates were included. This could help to improve maintainability and help viewers follow your reasoning.

Overall, great work! We are going to need all we have learned for years to come.
